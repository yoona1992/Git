...
namespace CCSIM.DAL.Base
{
    public class DbBase
    {
        ...
        public List<TModel> Query<TModel>(string sql, List<OracleParameter> parms, CommandType cmdType = CommandType.Text)
        {
            //存储过程（exec getActionUrlId @name,@ID）
            ...
            if (cmdType == CommandType.StoredProcedure)
            {
                ...
                sql = paraNames.Length > 0 ? $"begin {sql} ( {paraNames.ToString().Trim(',')});end;" : $"begin {sql};end;";
            }
            var list = SlaveDb.Database.SqlQuery<TModel>(sql, parms.ToArray());
            var enityList = list.ToList();
            return enityList;
    ...
    public class DbBase<T> : DbBase where T : class, new()
    {
        ...
        {
            MasterDb.Set<T>().Where(whereLambda).DeleteFromQuery();
        }
        ...
        {
            MasterDb.Set<T>().Where(whereLambda).UpdateFromQuery(updateExpression);
        }
